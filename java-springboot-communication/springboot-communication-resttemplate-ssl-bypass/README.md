...
##### Goal
The goal of this project is to present how to implement **communication** between two applications using **RestTemplate** secured by **SSL** - in this example SSL will be **bypassed**. The result is **Hello World** message and **port** numbers of **Text** and **Display** services in a **JSON** format in a **browser** using **Java** applications with **Spring Boot** framework.

##### Details
There are three ways how you can handle SSL in RestTemplete:
* Bypass it: in this option SSL checking is turned off
* Application: in this option SSL is checking in application
* JDK: in this option certificate is added to JDK and developer doesn't have to care of it 

In this example RestTemplate bypasses SSL. The application displays message "Hello World" and "port" numbers of "Text" and "Display" services in JSON format in a browser. Service "Text" provides text which should be displayed. Service "Display" connects with service "Text" and takes from it text and port and then display it in a browser.
...
* **Download** source code and open any **Command Line** tool on **project's folder**. You can do it in following way:
    ...
    * Go to source code folder with `cd chrisblog-it\java-springboot-communication\springboot-communication-resttemplate-ssl-bypass`
